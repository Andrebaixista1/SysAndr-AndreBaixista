...
def cadastrar(arq, data, patrimônio, marca, modelo, cor, status):
    try:
        a = open(arq, 'at') # 'at' = append
    except:
        ...
        try:
            a.write(f'{data};{patrimônio};{marca};{modelo};{cor};{status}\n')
        except:
            print('Houve um erro na hora de escrever os dados !')
        else:
...
def ProcurarPatrimonio(arq, patrimonio):
    try:
        ...
        print(f'{"Data":^20}{"Patrimônio":^10}{"Marca":^10}{"Modelo":^10}{"Cor":>15}{"Status":^20}')
        for dado in lista:
            dado[5] = dado[5].replace('\n', '')
            print(f'{dado[0]:<3}{dado[1]:^10}{dado[2]:^13}{azul}{dado[3]:<20}\033[m{dado[4]:<10}{amarelo}{dado[5]:<10}\033[m')
